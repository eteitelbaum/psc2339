...
## The Inflation-Unemployment Tradeoff {.smaller}

```{r}
#| fig-width: 8
#| fig-height: 4.5

x <- seq(2, 9, length.out = 200)
pc_df <- data.frame(x = x, y = 9 / (x - 1))

ggplot(pc_df, aes(x, y)) +
  geom_line(color = "#4472C4", linewidth = 1.5) +
  annotate("point", x = 3,   y = 4.5, size = 4, color = "#C00000") +
  annotate("text",  x = 3.3, y = 4.8, label = "A\n(low unemployment,\nhigh inflation)",
           size = 3.5, hjust = 0) +
  annotate("point", x = 7,   y = 1.5, size = 4, color = "#C00000") +
  annotate("text",  x = 6,   y = 1.1, label = "B\n(high unemployment,\nlow inflation)",
           size = 3.5, hjust = 0) +
  annotate("text",  x = 8.8, y = 3.2, label = "Phillips\nCurve",
           color = "#4472C4", size = 4, hjust = 0.5) +
  scale_x_continuous(limits = c(1, 10), breaks = NULL, expand = c(0, 0.3)) +
  scale_y_continuous(limits = c(0, 8),  breaks = NULL, expand = c(0, 0.3)) +
  labs(x = "Unemployment  →", y = "Inflation  →",
       caption = "Phillips (1958): lower unemployment tends to coincide with higher inflation, and vice versa") +
...
## Why No Output Cost? The Long-Run Phillips Curve {.smaller}
...
nat_rate <- 5
x <- seq(2, 8.5, length.out = 200)

srpc1 <- data.frame(x = x, y = 0.5 + 7 / (x - 1), label = "SRPC\u2081 (\u03c0\u1d49 low)")
srpc2 <- data.frame(x = x, y = 3.0 + 7 / (x - 1), label = "SRPC\u2082 (\u03c0\u1d49 higher)")
srpcs <- rbind(srpc1, srpc2)
srpcs <- srpcs[srpcs$y >= 0 & srpcs$y <= 10, ]

ggplot() +
  geom_line(data = srpcs, aes(x, y, color = label), linewidth = 1.3) +
  geom_segment(aes(x = nat_rate, xend = nat_rate, y = 0.2, yend = 9.5),
               color = "grey30", linewidth = 1.4, linetype = "dashed") +
  # Equilibrium points
  annotate("point", x = nat_rate, y = 1.9, size = 4) +
  annotate("text",  x = nat_rate + 0.25, y = 1.9, label = "A", size = 4.5, fontface = "bold", hjust = 0) +
  annotate("point", x = 3.1, y = 3.3, size = 4, color = "#C00000") +
  annotate("text",  x = 2.6, y = 3.5, label = "B", size = 4.5, fontface = "bold", color = "#C00000") +
  annotate("point", x = nat_rate, y = 4.4, size = 4) +
  annotate("text",  x = nat_rate + 0.25, y = 4.4, label = "C", size = 4.5, fontface = "bold", hjust = 0) +
  # Adjustment arrows
  annotate("segment", x = nat_rate - 0.1, y = 2.0, xend = 3.3, yend = 3.2,
           arrow = arrow(length = unit(0.25, "cm"), type = "closed"), color = "grey40") +
  annotate("segment", x = 3.1, y = 3.5, xend = nat_rate - 0.15, yend = 4.3,
           arrow = arrow(length = unit(0.25, "cm"), type = "closed"), color = "grey40") +
  # LRPC label
  annotate("text", x = nat_rate + 0.2, y = 9.7, label = "LRPC", size = 4,
           fontface = "bold", hjust = 0) +
  scale_color_manual(values = c("#2166ac", "#d7191c")) +
  scale_x_continuous(limits = c(1, 9), breaks = nat_rate,
                     labels = "Natural\nrate") +
  scale_y_continuous(limits = c(0, 10), breaks = NULL) +
  labs(x = "Unemployment", y = "Inflation  →", color = NULL,
       caption = "Friedman (1968): the short-run tradeoff vanishes as expectations adjust — the long-run curve is vertical") +
  theme_minimal(base_size = 13) +
  theme(panel.grid    = element_blank(),
        axis.line     = element_line(color = "grey40"),
        axis.title    = element_text(size = 12),
