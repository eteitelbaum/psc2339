...
ggplot(pct_regimes, aes(x = year, y = percent, color = regime_type)) +
  ...
  labs(x = NULL, y = "Percent",
       title = paste0("Trends in Regime Type, 1975 - ", latest_vdem_year, "\n")) +
...
top10_autocratizing |>
  gt() |>
  tab_header(
    title = paste0("Top 10 Autocratizing Countries, ", start_year, " - ", latest_vdem_year)
  ) |>
  fmt_number(
    columns = c(Change, `LDI Start`, `LDI End`),
    decimals = 2
  ) |>
  tab_style(
    style = list(cell_fill(color = "#9970ab"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime Start`, rows = `Regime Start` == "Liberal Democracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#de77ae"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime Start`, rows = `Regime Start` == "Electoral Democracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#fdb863"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime Start`, rows = `Regime Start` == "Electoral Autocracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#8c510a"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime Start`, rows = `Regime Start` == "Closed Autocracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#9970ab"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime End`, rows = `Regime End` == "Liberal Democracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#de77ae"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime End`, rows = `Regime End` == "Electoral Democracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#fdb863"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime End`, rows = `Regime End` == "Electoral Autocracy")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#8c510a"), cell_text(color = "white", size = px(14))),
    locations = cells_body(columns = `Regime End`, rows = `Regime End` == "Closed Autocracy")
  ) |>
  tab_footnote(
    footnote = "LDI = Liberal Democracy Index",
    locations = cells_column_labels(columns = c(`LDI Start`, `LDI End`))
  ) |>
  tab_source_note(
    source_note = "Source: V-Dem Institute, Varieties of Democracy Dataset"
  ) |>
  tab_source_note(
    source_note = "Note: Regime types are based on passing separate thresholds for elections and liberal rights (not the LDI score)."
  ) |>
  tab_style(
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(Change, `LDI Start`, `LDI End`))
  ) |>
  tab_style(
    style = cell_text(align = "center"),
    locations = cells_column_labels(columns = c(Change, `LDI Start`, `LDI End`))
  ) |>
  tab_options(
...
-   Explore the [Democracy Around the World](https://emmanuelteitelbaum.shinyapps.io/vdemdashboard/) App

- Discuss
  - Pick a backsliding country
  - How would you explain its democratic decline 