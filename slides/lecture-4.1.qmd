...
year_note <- function(dates) {
  years <- if (is.numeric(dates)) dates else suppressWarnings(as.integer(dates))
  years <- years[!is.na(years)]
  if (length(years) == 0) {
    return("Data are from the most recent available year (n/a)")
  }
  if (min(years) == max(years)) {
    return(paste("Data are from the most recent available year (", max(years), ")", sep = ""))
  }
  paste0(
...
wrap_caption <- function(text) {
  stringr::str_wrap(text, width = 110)
}

source_overrides <- c(
  SP.POP.BRTH.MF = "World Bank staff estimates (UN World Population Prospects 2019 Revision)",
  SP.POP.1564.TO.ZS = "World Bank staff estimates (UN World Population Prospects 2019 Revision)",
  SP.POP.DPND = "World Bank staff estimates (UN World Population Prospects 2019 Revision)",
  SP.POP.DPND.OL = "World Bank staff estimates (UN World Population Prospects 2019 Revision)",
  SP.POP.DPND.YG = "World Bank staff estimates (UN World Population Prospects 2019 Revision)",
  SP.DYN.TFRT.IN = "UN Population Division, World Population Prospects 2019 Revision",
  SL.TLF.CACT.FE.ZS = "International Labour Organization, ILOSTAT database",
  SG.GEN.PARL.ZS = "Inter-Parliamentary Union (IPU)",
  SG.LAW.INDX = "World Bank, Women, Business and the Law",
  SP.M18.2024.FE.ZS = "World Bank, World Development Indicators",
  SG.VAW.1549.ZS = "World Bank, World Development Indicators"
)

build_caption <- function(indicator_id, source_org, dates) {
  source_text <- source_org
  if (indicator_id %in% names(source_overrides)) {
    source_text <- source_overrides[[indicator_id]]
  }
  source_line <- wrap_caption(paste("Source:", source_text))
  paste(source_line, year_note(dates), sep = "\n")
}
```
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    ...
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, sex_ratio_data$date)
  )
```
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, wbl_data$date)
  )
```
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, child_marriage_data$date)
  )
```
...
domestic_violence_data <- wb_data(
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, domestic_violence_data$date)
  )
```
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, working_age_pop_data$date)
  )
```
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, dependency_ratio_data$date)
  )
```
...
dependency_ratio_old_data <- wb_data(
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, dependency_ratio_old_data$date)
  )
```
...
dependency_ratio_yng_data <- wb_data(
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, dependency_ratio_yng_data$date)
  )
```
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, flfp_data$date)
  )
```
...
flfp_gdp_note <- paste("Source: World Bank Development Indicators.", year_note(flfp_gdp$date))
...
ggplotly(flfp_vs_gdp, tooltip = c("country", "flfp", "gdp_pc")) %>%   
  # add source
  layout(annotations = list(text = flfp_gdp_note,
                            font = list(size = 10), showarrow = FALSE,
                            xref = 'paper', x = 0, xanchor = 'left',
                            yref = 'paper', y = -.1, yanchor = 'auto', yshift = 0,
                            align = 'left')) %>%
  layout(margin = list(l = 60, r = 40, t = 40, b = 110)) %>%
  layout(xaxis = list(title = list(standoff = 20))) %>%
  # add web address
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, tfr_data$date)
  )
```
...
women_representation_data <- wb_data(
...
ne_countries(returnclass = "sf") %>%
  ...
  labs(
    title = indicator_info$indicator,
    fill = NULL,
    caption = build_caption(indicator_info$indicator_id, indicator_info$source_org, women_representation_data$date)
  )
```
